...
截至 2026-08-17，PR #10 已通过普通 merge commit 合并到 `main`，对应提交为 `511712f`。当前版本为可在本地运行和验收的 React + Django + MySQL MVP，尚未部署到正式服务器。

当前支持中英文界面、公开作品、制作日志、询价购物车、询价、离线订单、国际结账信息基础、报价接受或拒绝、客户私人项目、进度图片、留言、确认回执、评价提交与审核，以及 Django Admin 管理后台。

当前暂不包含真实在线支付、短信验证码、微信登录、摄像头直播和正式服务器部署。

第一次接触本项目，或准备换电脑演示，请先完整阅读 [电脑小白启动与换机演示说明书](docs/BEGINNER_STARTUP_GUIDE.md)。该文档分别说明了“只从 GitHub 启动全新环境”和“连同本机数据库、图片、账号一起迁移”两种流程。
...
网站界面支持简体中文和英文切换，公开作品与制作日志可在后台维护可选英文内容；国际化交易的数据边界和未来真实支付接入要求详见 [中英文与国际化交易基础](docs/INTERNATIONAL_COMMERCE.md)。当前仍不会连接任何真实支付渠道或执行自动换汇。
...
报价接受后的国际结账确认、金额构成、私有收货地址、历史订单兼容和未来真实支付适配边界详见 [国际结账基础](docs/INTERNATIONAL_CHECKOUT.md)。当前仍未连接任何真实支付渠道。
