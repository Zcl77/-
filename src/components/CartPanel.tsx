  ...
  return (
    <div className="fixed inset-0 z-[70]" role="dialog" aria-modal="true" aria-label={t('询价购物车')}>
      <button
        ...
        aria-label={t('关闭')}
      />
      <aside className="absolute right-0 top-0 flex h-full w-full max-w-md flex-col border-l border-studio-line bg-studio-canvas p-5 shadow-2xl sm:p-7">
        ...
        {items.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center text-center">
            <ShoppingBag className="h-10 w-10 text-studio-brass" />
          ...
          <>
            <div className="flex-1 space-y-3 overflow-y-auto py-5">
              {items.map((item) => (
                <div key={item.id} className="flex items-center gap-3 border-b border-studio-line pb-3">
                  <img src={item.coverUrl} alt="" className="h-16 w-20 rounded-[3px] object-cover" />
